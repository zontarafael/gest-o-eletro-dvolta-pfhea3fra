...
//   despesas_adicionais: numeric (nullable, default: 0)
//   valor_site: numeric (nullable, default: 0)
//   taxa_site: numeric (nullable, default: 0)
//   link_site: text (nullable)
//   despesas_detalhadas: jsonb (nullable, default: '[]'::jsonb)
//   impostos_detalhados: jsonb (nullable, default: '[]'::jsonb)
//   custo_final: numeric (nullable, default: 0)
// Table: profiles
//   id: uuid (not null)
//   email: text (not null)
//   name: text (not null)
...
//   created_at: timestamp with time zone (nullable, default: now())
//   role: text (nullable, default: 'visitante'::text)
// Table: venda_itens
//   id: uuid (not null, default: gen_random_uuid())
...
// Table: empresa_config
//   PRIMARY KEY empresa_config_pkey: PRIMARY KEY (id)
//   FOREIGN KEY empresa_config_user_id_fkey: FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE
//   UNIQUE empresa_config_user_id_key: UNIQUE (user_id)
// Table: fornecedores
...
// Table: clientes
//   Policy "clientes_policy_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (get_user_role() = ANY (ARRAY['admin'::text, 'colaborador'::text]))
//   Policy "clientes_policy_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: empresa_config
//   Policy "empresa_config_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (user_id = auth.uid())
//   Policy "empresa_config_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "empresa_config_update" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (user_id = auth.uid())
//     WITH CHECK: (user_id = auth.uid())
// Table: fornecedores
//   Policy "fornecedores_policy_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (get_user_role() = ANY (ARRAY['admin'::text, 'colaborador'::text]))
//   Policy "fornecedores_policy_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: movimentacoes_financeiras
//   Policy "financeiro_policy_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (get_user_role() = 'admin'::text)
// Table: produtos
//   Policy "produtos_policy_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (get_user_role() = ANY (ARRAY['admin'::text, 'colaborador'::text]))
//   Policy "produtos_policy_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: profiles
//   Policy "profiles_policy" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((id = auth.uid()) OR (get_user_role() = 'admin'::text))
//     WITH CHECK: ((id = auth.uid()) OR (get_user_role() = 'admin'::text))
// Table: venda_itens
//   Policy "venda_itens_policy_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (get_user_role() = ANY (ARRAY['admin'::text, 'colaborador'::text]))
//   Policy "venda_itens_policy_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: vendas
//   Policy "vendas_policy_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (get_user_role() = ANY (ARRAY['admin'::text, 'colaborador'::text]))
//   Policy "vendas_policy_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
...
//     VALUES (NEW.id, NEW.email, COALESCE(NEW.raw_user_meta_data->>'name', 'Usuário'), 'visitante')
//     ON CONFLICT (id) DO NOTHING;
//     RETURN NEW;
//   END;
//   $function$
//
// FUNCTION handle_venda_item_stock()
...
// --- TRIGGERS ---
// Table: venda_itens
//   on_venda_item_change: CREATE TRIGGER on_venda_item_change AFTER INSERT OR DELETE OR UPDATE ON public.venda_itens FOR EACH ROW EXECUTE FUNCTION handle_venda_item_stock()

// --- INDEXES ---
// Table: empresa_config
//   CREATE UNIQUE INDEX empresa_config_user_id_key ON public.empresa_config USING btree (user_id)
